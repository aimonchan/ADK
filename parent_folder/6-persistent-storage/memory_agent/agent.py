...
def update_reminder(index: int, updated_text: str, tool_context: ToolContext) -> dict:
    ...
    #Update state with the modefied list
    tool_context.state["reminders"] = reminders
    return {
        "action": "update_reminder",
        "index": index,
...
def delete_reminder(index: int, tool_context: ToolContext) -> dict:
    ...
    Args:
        index: The 1-based index of the reminder to delete_reminder
        tool_context: Context for accessing and updating session state
...
memory_agent = Agent(
    ...
    You can help users manage thier reminders with the following capabilities:
    1. Add new reminders
    ...
    Always be friendly and address the user by name. If you don't know thier name yet, use the update_user_name tool to store it when they introduce themselves.
    ...
    **REMINDER MANAGEMENT GUIDELINES:**
    ...
    5. For addition:
        ...
        - Focus on the task itself (e.g., "add a reminder to buy milk")
    ...
    - You don't have to be 100% correct, buty try to be as close as possible.
