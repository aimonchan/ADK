...
def before_tool_callback(
    ...
    # Skip the call completely for restricted countires 
    if(
...
def after_tool_callback(
    ...
    """
    Simple callback that modifies that tool response after execution.
    """
    tool_name = tool.name 
    ...
    if tool_name == "get_capital_city" and "washington" in original_result.lower():
        ...
        modified_response["result"] = (
            f"{original_result} (Note: This is the capital of the USA. us)"
        )
    ...
    print(f"[Callback] No modifications needed, returning original response")
    return None
